...
bool sudoku_game(int len) {
    ...
    while(1) {
        ...
        if(choice == 'y') {
            ...
            get_saved_sudoku(seed_sudoku, problem_sudoku, &sudokusize, file_name);
            if(sudokusize != len) {
    ...
    while(!quit) {
        ...
        switch(cmd) {
            ...
            puts("Save file as");
            scanf("%s", file_name);
            if(save_sudoku_to_file(seed_sudoku, sudoku, len, file_name)) {
            ...
            case 4:
                ...
                printf("%d at position %c, %c\n", seed_sudoku[row][col], row + 'a', col + 'a');
                break;
            case 5:
            ...
            case 9:
                clr_scr();
                if(get_data(command, strlen(command), &data)) {
                    if(data.num > len) {
                        print_game_commands();
                        print_sudoku_game(seed_sudoku, sudoku, len);
